...
string unHash(string hash){
	...
	string ip = conv.substr(0,conv.find(":"));
	return ip;
...
int main(int argc , char *argv[]){
	...
	if(argc < 2){
		...
		cout<<"./executable {hash} {username} {passwd} {amount}"<<endl;
	...
	string myuser = argv[2];
	string mypassword = argv[3];
	vector<string>info = myInfo(myuser,mypassword);
	
	
	string om = unHash(address);
	...
	getaddrinfo(om.c_str(),PORT,&hints,&result);
	...
	// anyway , finishing this and going to end for today
	address+="\n";
	string packet = address+info[2]+"\n"+":"+amount;
	...
	if(choice.compare("yes")==0 || choice.compare("Yes")==0 || choice.compare("Y") ==0 || choice.compare("y") == 0){
		...
		if(curr <= 0){
			...
			string rac;
			calc>>rac;
			flyCoinProtocol(myuser,mypassword,"w",rac);
